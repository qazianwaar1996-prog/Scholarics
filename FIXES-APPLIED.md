# Fixes Applied & Verified — study-metricsv.2
...
| `index.html` (sm2) hamburger | works | **still works ✅** |
...
### 3 JavaScript files
1. **`js/sm-shell.js`** — (a) marks the toggle `toggle.dataset.smBound='1'` after binding so premium.js can't double-bind it; (b) adds `.sm-install-btn` to the ≤860px nav hide-rule so the PWA install button can't overflow the mobile header.
2. **`js/premium.js`** — guards the `#menuToggle` binding: `if (toggle && navLinks && !toggle.dataset.smBound)`.
...
- Everything else in the repo is **byte-identical** to your GitHub `main` — including `css/`, images, `manifest.json`, `sw.js`, `server.js`, docs, and the 6 sm2 pages' footers.
...
`body.premium` to every non-sm2 page, which switched on the legacy purple/glass
skin in `css/calculators.css` + `css/studymetrics-v2.css` (focus rings, field
labels, grade letters, result heros all turned purple `#a78bfa`/`rgba(124,58,237)`).
The homepage (sm2) never gets that class, so it stayed gold/black.
...
1. **`js/premium.js`** — removed the `document.body.classList.add('premium')`
   ...
   (The earlier `dataset.smBound` hamburger guard is retained.)
...
- Hamburger: **0 failures across all 50 old-nav pages** (6 sm2 pages use the
...
returning visitors had the **old** `sm-shell.js`/`premium.js` (pre-fix) stuck in
the Service Worker cache — `sw.js` serves JS **cache-first** (`sm-static-v2.4`)
...
to do nothing. The actual reason: `sm-shell.js` injected
...
**Fix:** `js/sm-shell.js` — removed `overflow:hidden` from the injected
...
left). The only remaining difference vs the homepage `.sm2-tool` was a missing
resting shadow.
