(function () {
  ...
  function validateRow(r) {
    ...
    var errors = [];
    if (r.weight !== '' && r.weight !== undefined && (isNaN(w) || w < 0)) {
      errors.push('Weight must be 0–100');
    } else if (!isNaN(w) && w > 100) {
      errors.push('Weight cannot exceed 100%');
    }
    if (r.score !== '' && r.score !== undefined && (isNaN(s) || s < 0)) {
      errors.push('Score must be non-negative');
    } else if (!isNaN(s) && s > 200) {
      errors.push('Score seems too large (max 200)');
    }
    return errors;
  }
  function compute() {
