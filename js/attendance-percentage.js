...
function compute(){
  ...
  if(att < 0) att = 0;
  if(tot < 0) tot = 0;
  if(att > tot && !isNaN(att) && !isNaN(tot) && tot > 0){
    if(pctEl) pctEl.textContent='—';
    if(statusEl) statusEl.textContent='Attended cannot exceed total classes';
    if(vEl){ vEl.className='verdict bad'; vEl.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6M9 9l6 6"/></svg><div><b>Invalid input</b> Attended classes cannot exceed total classes held.</div>'; }
    return;
  }
