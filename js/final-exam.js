(function () {
  ...
  function compute() {
    ...
    // Inline validation
    var inputValid = true;
    [curEl, weightEl].forEach(function(el) {
      if (el) { el.classList.remove('input-error'); var e = el.parentNode && el.parentNode.querySelector('.field-error-msg'); if (e) e.remove(); }
    });

    if (!isNaN(cur) && (cur < 0 || cur > 150)) {
      curEl.classList.add('input-error');
      var e1 = document.createElement('span'); e1.className = 'field-error-msg'; e1.textContent = 'Current grade must be 0–150';
      if (curEl.parentNode) curEl.parentNode.appendChild(e1);
      inputValid = false;
    }
    if (!isNaN(weight) && (weight <= 0 || weight > 100)) {
      weightEl.classList.add('input-error');
      var e2 = document.createElement('span'); e2.className = 'field-error-msg'; e2.textContent = 'Weight must be 1–100%';
      if (weightEl.parentNode) weightEl.parentNode.appendChild(e2);
      inputValid = false;
    }

    if (isNaN(cur) || isNaN(goal) || isNaN(weight) || weight <= 0 || weight > 100 || !inputValid) {
