(function () {
  ...
  function compute() {
    ...
    var targetPct = parseFloat(targetRaw) || 0;

    // Validate target percentage
    if (targetEl) {
      targetEl.classList.remove('input-error');
      var errEl = targetEl.parentNode && targetEl.parentNode.querySelector('.field-error-msg');
      if (errEl) errEl.remove();

      if (targetRaw !== '' && (isNaN(parseFloat(targetRaw)) || parseFloat(targetRaw) < 0 || parseFloat(targetRaw) > 100)) {
