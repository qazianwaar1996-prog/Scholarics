/* ==========================================================================
   Scholarics v2.0 — Feature Engine
...
(function () {
  ...
  function gpaData() {
    var rows = store.get('sc_gpa_rows', []);
    if (!rows.length) return null;
    var scale = store.get('sc_gpa_scale', 'letter') || 'letter';
    var pts = 0, cr = 0;
  ...
  function cgpaData() {
    var rows = store.get('sc_cgpa_rows', []);
    if (!rows.length) return null;
    var pts = 0, cr = 0;
    rows.forEach(function (r) {
  ...
  function semGpaData() {
    var rows = store.get('sc_semester_gpa', []);
    if (!rows.length) return null;
    var pts = 0, cr = 0;
    rows.forEach(function (r) {
  ...
  function createModal(id, content, maxW) {
    ...
    overlay.style.cssText = [
      ...
      'padding:16px;animation:scFadeIn .18s ease',
    ].join(';');
    ...
    if (!document.getElementById('scModalAnim')) {
      var s = document.createElement('style');
      s.id = 'scModalAnim';
      s.textContent = '@keyframes scFadeIn{from{opacity:0}to{opacity:1}}';
  ...
  function createDropdown(anchor, content, width) {
    ...
    dd.style.cssText = [
      ...
      'animation:scFadeIn .14s ease;overflow:hidden',
    ].join(';');
  ...
  function closeAllDropdowns() {
    document.querySelectorAll('.sc2-dropdown').forEach(function (d) { d.remove(); });
  }
  ...
  ══════════════════════════════════════════════════════════════════════ */
  function initSearch() {
    /* Ensure animation keyframe exists before dropdown is created */
    if (!document.getElementById('scModalAnim')) {
      var _ks = document.createElement('style');
      _ks.id = 'scModalAnim';
      _ks.textContent = '@keyframes scFadeIn{from{opacity:0;transform:translateY(-4px)}to{opacity:1;transform:none}}';
    ...
    function wireSearchInput(input) {
      ...
      function doSearch(q) {
        ...
        html += '<div style="border-top:1px solid var(--border);padding:10px 16px">'
          + '<a href="academic-resources.html" style="font-size:12.5px;font-weight:600;color:var(--gold-strong,#a97c14);text-decoration:none">Browse all tools →</a></div>';

        var rect = input.getBoundingClientRect();
        closeAllDropdowns();
        dropdown = document.createElement('div');
        dropdown.className = 'sc2-dropdown';
        var ddWidth = Math.max(rect.width, 300);
        var ddLeft = rect.left;
        /* Clamp to viewport right edge */
        var vw = window.innerWidth || document.documentElement.clientWidth;
        if (ddLeft + ddWidth > vw - 12) { ddLeft = Math.max(8, vw - ddWidth - 12); }
    ...
    // Bind on existing inputs + any injected by sc-shell
    function bindAll() {
      document.querySelectorAll('#sc2Search').forEach(wireSearchInput);
    }
    bindAll();
    // ⌘K / Ctrl+K shortcut
    document.addEventListener('keydown', function (e) {
      if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
        e.preventDefault();
        var inp = document.querySelector('#sc2Search');
  ...
  /* ══════════════════════════════════════════════════════════════════════
     2. NOTIFICATIONS
  ══════════════════════════════════════════════════════════════════════ */
  var NOTIF_KEY = 'sc_notifs_dismissed';
  var NOTIFS = [
  ...
  function initNotifications() {
    function bindBtn(btn) {
      ...
      var dot = btn.querySelector('.sc2-dot');
        ...
        if (!active.length) {
          ...
          active.forEach(function (n) {
            html += '<div style="padding:14px 18px;border-bottom:1px solid var(--border-2,var(--border));display:flex;gap:12px;align-items:flex-start">'
              ...
              + '</div>'
              + '<button onclick="SC2Features.dismissNotif(\'' + n.id + '\',this)" style="background:none;border:none;cursor:pointer;color:var(--ink-3);font-size:16px;padding:2px 4px;line-height:1">×</button>'
              + '</div>';
          });
          html += '<div style="padding:10px 18px"><button onclick="SC2Features.dismissAllNotifs(this)" style="background:none;border:none;cursor:pointer;font-size:12.5px;font-weight:600;color:var(--ink-3)">Clear all</button></div>';
        }
    ...
    window.SC2Features = window.SC2Features || {};
    window.SC2Features.dismissNotif = function (id, el) {
      ...
      document.querySelectorAll('[aria-label="Notifications"] .sc2-dot').forEach(function (dot) {
        var remaining = NOTIFS.filter(function (n) { return d.indexOf(n.id) === -1; });
        dot.style.display = remaining.length ? '' : 'none';
      });
    };
    window.SC2Features.dismissAllNotifs = function (el) {
      var ids = NOTIFS.map(function (n) { return n.id; }); store.set(NOTIF_KEY, ids);
      closeAllDropdowns();
      document.querySelectorAll('[aria-label="Notifications"] .sc2-dot').forEach(function (dot) { dot.style.display = 'none'; });
    };
  }
  ...
  function initProfile() {
    ...
    function openProfile() {
      var name = store.get('sc_dash_name', '') || '';
      ...
      var html = '<div style="padding:28px 28px 24px">'
        ...
        + '<button onclick="this.closest(\'[id^=sc2modal]\').remove()" style="background:none;border:none;cursor:pointer;font-size:20px;color:var(--ink-3);line-height:1">×</button>'
        + '</div>'
        ...
        + '<input id="sc2ProfName" class="input" type="text" value="' + esc(name) + '" placeholder="Your name" style="width:100%;padding:10px 14px;border:1px solid var(--border);border-radius:12px;font-size:14px;color:var(--ink);background:var(--card-2,#faf9f5);box-sizing:border-box">'
        + '</div>'
        + '<div>'
        ...
        + '<button onclick="SC2Features.setTheme(\'light\',this)" class="sc2-btn sc2-btn-ghost" style="flex:1;justify-content:center;font-size:12.5px">☀️ Light</button>'
        + '<button onclick="SC2Features.setTheme(\'dark\',this)" class="sc2-btn sc2-btn-ghost" style="flex:1;justify-content:center;font-size:12.5px">🌙 Dark</button>'
        + '<button onclick="SC2Features.setTheme(\'system\',this)" class="sc2-btn sc2-btn-ghost" style="flex:1;justify-content:center;font-size:12.5px">⚙️ System</button>'
        + '</div>'
        + '</div>'
        + '<button onclick="SC2Features.saveProfile()" class="sc2-btn sc2-btn-primary" style="width:100%;justify-content:center;margin-top:4px">Save Profile</button>'
        + '</div>'
        + '</div>';
      var m = createModal('sc2modalProfile', html, '420px');
      m.overlay.id = 'sc2modalProfile';
    }
    function bindAll() {
    ...
    window.SC2Features = window.SC2Features || {};
    window.SC2Features.setTheme = function (t) {
      var theme = t === 'system' ? (window.matchMedia && matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light') : t;
      document.documentElement.setAttribute('data-theme', theme);
      try { localStorage.setItem('sc_theme', t === 'system' ? '' : theme); } catch (e) {}
      SC.toast(t === 'system' ? '⚙️ Using system theme' : (theme === 'dark' ? '🌙 Dark mode on' : '☀️ Light mode on'), 'info');
    };
    window.SC2Features.saveProfile = function () {
      var inp = document.getElementById('sc2ProfName');
      if (!inp) return;
      var n = inp.value.trim().slice(0, 40);
      store.set('sc_dash_name', n);
      SC.toast('Profile saved ✓', 'success');
      // update sidebar name
      document.querySelectorAll('.sc2-side-foot .nm').forEach(function (el) { el.textContent = n || 'Profile'; });
      var modal = document.getElementById('sc2modalProfile');
  ...
  function initWatchDemo() {
    document.addEventListener('click', function (e) {
      var btn = e.target.closest('a[href="ai.html"].sc2-btn-ghost, button.sc2-btn-ghost');
      if (!btn) return;
      ...
      var html = '<div style="padding:28px">'
        ...
        + '<h2 style="margin:0;font-size:18px;font-weight:700;color:var(--ink)">How Scholarics Works</h2>'
        + '<button onclick="document.getElementById(\'sc2modalDemo\').remove()" style="background:none;border:none;cursor:pointer;font-size:20px;color:var(--ink-3);line-height:1">×</button>'
        + '</div>'
        ...
        + '<a href="gpa.html" class="sc2-btn sc2-btn-primary" style="flex:1;justify-content:center" onclick="document.getElementById(\'sc2modalDemo\').remove()">Start with GPA →</a>'
        + '<a href="ai.html" class="sc2-btn sc2-btn-ghost" style="flex:1;justify-content:center" onclick="document.getElementById(\'sc2modalDemo\').remove()">Try AI Tutor</a>'
        + '</div>'
        + '</div>';
      var m = createModal('sc2modalDemo', html, '500px');
      m.overlay.id = 'sc2modalDemo';
    });
  }
  ...
  function initDashboardMetrics() {
    if (!document.querySelector('.sc2-metric')) return;
    ...
    function updateCard(el, val, ms, progPct) {
      ...
      var progEl = el.querySelector('.sc2-progress i');
      if (mvEl && val !== null) {
    ...
    // Card 1: Semester GPA
    var c1 = document.getElementById('db-metric-1') || document.querySelectorAll('.sc2-metric')[0];
    if (c1 && gd) {
    ...
    // Card 2: CGPA
    var c2 = document.getElementById('db-metric-2') || document.querySelectorAll('.sc2-metric')[1];
    if (c2 && cd) {
      updateCard(c2, cd.cgpa, gpaLabel(cd.cgpa) + ' · ' + cd.semesters + ' semester' + (cd.semesters !== 1 ? 's' : ''), (cd.cgpa / 4) * 100);
    }

    // Card 3: Target GPA
    var c3 = document.getElementById('db-metric-3') || document.querySelectorAll('.sc2-metric')[2];
    if (c3 && tgt) {
    ...
    // Card 4: Study Hours
    var c4 = document.getElementById('db-metric-4') || document.querySelectorAll('.sc2-metric')[3];
    if (c4 && hrs !== null) {
  ...
  ══════════════════════════════════════════════════════════════════════ */
  var GOALS_KEY = 'sc2_goals';
  var STREAK_KEY = 'sc2_streak';

  function defaultGoals() {
    var cd = cgpaData(); var ad = store.get('sc_attend', null);
    var hrs = studyHoursData();
    return [
  ...
  function initGoals() {
    ...
    var goalContainer = goalsSection.querySelector('.sc2-goal:not(.sc2-streak-mini)') ? goalsSection : null;
    ...
    function renderGoals() {
      ...
      var panelHead = goalsSection.querySelector('.sc2-panel-head');
      var streakMini = goalsSection.querySelector('.sc2-streak-mini');
      if (!panelHead || !streakMini) return;

      // Remove old goal divs
      goalsSection.querySelectorAll('.sc2-goal').forEach(function (el) { el.remove(); });

      // Update badge
      var badge = panelHead.querySelector('.sc2-pill-badge');
      if (badge) badge.textContent = goals.length + ' ACTIVE';

      // Inject goals before streak
      goals.forEach(function (g) {
        var div = document.createElement('div');
        div.className = 'sc2-goal';
        div.innerHTML = '<div class="sc2-goal-row">'
          + '<span>' + esc(g.label) + '</span>'
          + '<div style="display:flex;align-items:center;gap:8px">'
          + '<b>' + Math.min(100, g.current || 0) + '%</b>'
          + '<button onclick="SC2Features.editGoal(\'' + g.id + '\')" style="background:none;border:none;cursor:pointer;padding:2px 4px;color:var(--ink-3);font-size:13px;line-height:1" title="Edit goal">✎</button>'
          + '<button onclick="SC2Features.deleteGoal(\'' + g.id + '\')" style="background:none;border:none;cursor:pointer;padding:2px 4px;color:var(--ink-3);font-size:13px;line-height:1" title="Delete goal">×</button>'
          + '</div>'
          + '</div>'
          + '<div class="sc2-progress"><i style="width:' + Math.min(100, g.current || 0) + '%"></i></div>';
        goalsSection.insertBefore(div, streakMini);
      });

      // Add goal button
      var addBtn = goalsSection.querySelector('.sc2-add-goal-btn');
      if (!addBtn) {
        addBtn = document.createElement('button');
        addBtn.className = 'sc2-btn sc2-btn-ghost sc2-add-goal-btn';
        addBtn.style.cssText = 'width:100%;justify-content:center;font-size:12.5px;margin-top:8px';
        addBtn.textContent = '+ Add Goal';
        addBtn.addEventListener('click', function () { SC2Features.addGoal(); });
    ...
    window.SC2Features = window.SC2Features || {};
    window.SC2Features.editGoal = function (id) {
      ...
      var html = '<div style="padding:24px">'
        ...
        + '<div style="display:flex;gap:10px">'
        + '<button onclick="SC2Features.saveGoal(\'' + id + '\')" class="sc2-btn sc2-btn-primary" style="flex:1;justify-content:center">Save</button>'
        + '<button onclick="document.getElementById(\'sc2modalGoal\').remove()" class="sc2-btn sc2-btn-ghost" style="flex:1;justify-content:center">Cancel</button>'
        + '</div></div>';
      var m = createModal('sc2modalGoal', html, '380px');
      m.overlay.id = 'sc2modalGoal';
      document.getElementById('goalCurrent').addEventListener('input', function () {
        document.querySelector('#sc2modalGoal label:nth-of-type(2)').textContent = 'Progress (' + this.value + '%)';
      });
    };
    window.SC2Features.addGoal = function () {
      var html = '<div style="padding:24px">'
        ...
        + '<div style="display:flex;gap:10px">'
        + '<button onclick="SC2Features.saveGoal(null)" class="sc2-btn sc2-btn-primary" style="flex:1;justify-content:center">Add Goal</button>'
        + '<button onclick="document.getElementById(\'sc2modalGoal\').remove()" class="sc2-btn sc2-btn-ghost" style="flex:1;justify-content:center">Cancel</button>'
        + '</div></div>';
      var m = createModal('sc2modalGoal', html, '380px');
      m.overlay.id = 'sc2modalGoal';
    ...
    window.SC2Features.saveGoal = function (id) {
      ...
      var m = document.getElementById('sc2modalGoal'); if (m) m.remove();
      renderGoals(); SC.toast(id ? 'Goal updated ✓' : 'Goal added ✓', 'success');
    };
    window.SC2Features.deleteGoal = function (id) {
      var list = (store.get(GOALS_KEY, goals) || []).filter(function (g) { return g.id !== id; });
      store.set(GOALS_KEY, list); goals = list; renderGoals();
      SC.toast('Goal removed', 'info');
    };
  }
  ...
  ══════════════════════════════════════════════════════════════════════ */
  var DEADLINES_KEY = 'sc2_deadlines';
  ...
  function initDeadlines() {
    ...
    if (!panel) {
      document.querySelectorAll('.sc2-panel, .sc2-card').forEach(function (p) {
        if (!panel && p.querySelector('h3') && p.querySelector('h3').textContent.indexOf('Deadline') !== -1) panel = p;
      });
    }
    if (!panel) return;
    var section = panel.querySelector('.sc2-list');
    if (!section) { section = document.createElement('div'); section.className = 'sc2-list'; panel.appendChild(section); }
    var panelHead = panel.querySelector('.sc2-panel-head');
    if (!panelHead) return;
    ...
    function renderDeadlines() {
      ...
      sorted.slice(0, 5).forEach(function (dl) {
        ...
        row.className = 'sc2-list-row';
        row.innerHTML = '<span class="sc2-list-ico ' + (dl.done ? 'ico-ok' : days <= 1 ? 'ico-gold' : 'ico-info') + '" style="cursor:pointer" onclick="SC2Features.toggleDeadline(\'' + dl.id + '\')" title="' + (dl.done ? 'Mark undone' : 'Mark done') + '">'
          ...
          + '</span>'
          + '<div class="sc2-list-body"><div class="t" style="' + (dl.done ? 'text-decoration:line-through;opacity:.55' : '') + '">' + esc(dl.title) + '</div><div class="s">' + esc(dl.subject) + ' · Due ' + new Date(dl.due + 'T12:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric'}) + '</div></div>'
          + (tc ? '<span class="sc2-tag ' + tc + '">' + tagLabel(days, dl.done) + '</span>' : '')
          + '<button onclick="SC2Features.deleteDeadline(\'' + dl.id + '\')" style="background:none;border:none;cursor:pointer;color:var(--ink-3);font-size:16px;padding:4px;line-height:1;margin-left:4px" title="Delete">×</button>';
        section.appendChild(row);
      });

      // Add deadline button
      if (!panel.querySelector('.sc2-add-deadline-btn')) {
        var addBtn = document.createElement('button');
        addBtn.className = 'sc2-btn sc2-btn-ghost sc2-add-deadline-btn';
        addBtn.style.cssText = 'width:100%;justify-content:center;font-size:12.5px;margin-top:10px';
        addBtn.textContent = '+ Add Deadline';
        addBtn.addEventListener('click', function () { SC2Features.addDeadline(); });
    ...
    window.SC2Features = window.SC2Features || {};
    window.SC2Features.toggleDeadline = function (id) {
    ...
    window.SC2Features.deleteDeadline = function (id) {
      var list = (store.get(DEADLINES_KEY, deadlines) || []).filter(function (dl) { return dl.id !== id; });
      store.set(DEADLINES_KEY, list); deadlines = list; renderDeadlines();
      SC.toast('Deadline removed', 'info');
    };
    window.SC2Features.addDeadline = function () {
      ...
      var html = '<div style="padding:24px">'
        ...
        + '<button onclick="SC2Features.saveDeadline()" class="sc2-btn sc2-btn-primary" style="flex:1;justify-content:center">Add Deadline</button>'
        + '<button onclick="document.getElementById(\'sc2modalDL\').remove()" class="sc2-btn sc2-btn-ghost" style="flex:1;justify-content:center">Cancel</button>'
        + '</div></div></div>';
      var m = createModal('sc2modalDL', html, '380px');
      m.overlay.id = 'sc2modalDL';
    };
    window.SC2Features.saveDeadline = function () {
      ...
      if (!title.trim() || !due) { SC.toast('Please fill in title and date', 'error'); return; }
      var list = store.get(DEADLINES_KEY, deadlines) || [];
      ...
      var m = document.getElementById('sc2modalDL'); if (m) m.remove();
      renderDeadlines(); SC.toast('Deadline added ✓', 'success');
    };
  }
  ...
  function initGpaTrend() {
    var trendSvg = document.querySelector('.sc2-trend svg');
    var trendSum = document.querySelector('.sc2-trend-sum');
    ...
    // Update labels
    var labels = document.querySelector('.sc2-trend + div');
    if (labels) {
  ...
  function initCalendar() {
    var calSection = document.querySelector('.sc2-cal');
    if (!calSection) return;
    var panelHead = calSection.closest('.sc2-panel').querySelector('.sc2-panel-head h3');
  ...
  function initAiSuggestions() {
    ...
    if (!sugContainer) {
      var firstSug = document.querySelector('.sc2-sug');
    ...
    var html = sugs.map(function (s) {
      return '<div class="sc2-sug"><span class="si"><svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" style="width:14px;height:14px;display:inline;color:var(--gold-strong)"><path d="M12 3l1.6 5.4L19 10l-5.4 1.6L12 17l-1.6-5.4L5 10l5.4-1.6L12 3Z"/></svg></span><div class="st"><b>' + s.icon + ' ' + esc(s.title) + ':</b> ' + esc(s.body) + '</div></div>';
    }).join('');
    var existing = sugContainer.querySelectorAll('.sc2-sug');
    existing.forEach(function (el) { el.remove(); });
    var askBtn = sugContainer.querySelector('.sc2-btn');
  ...
  function init() {
    ...
    if (document.querySelector('.sc2-metric, .sc2-goal, .sc2-cal')) {
    ...
    setTimeout(function () {
      var n = SC.store.get('sc_dash_name', '');
      if (n) document.querySelectorAll('.sc2-side-foot .nm').forEach(function (el) { el.textContent = n; });
    }, 300);
  }

  /* Boot: wait for both DOM and SC to be ready */
  function boot() {
    if (typeof window.SC === 'undefined') {
      /* SC not ready — poll with backoff (handles dynamic load on tool pages) */
      var attempts = 0;
