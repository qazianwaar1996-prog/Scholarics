...
(function () {
  ...
  function printReport() {
    ...
    var rows = [];
    semesters.forEach(function (s) {
      var sg = CORE.calcGpa(scale, s.courses);
      rows.push("<tr><td colspan='4' style='background:#f6f5f2;font-weight:700;padding:8px 12px;font-size:13px'>" +
        esc(s.name) + (sg.gpa !== null ? " — GPA: " + fmtGpa(sg.gpa) : "") + "</td></tr>");
      s.courses.forEach(function (c) {
        rows.push("<tr><td style='padding:7px 12px'>" + esc(c.name || "—") + "</td><td>" + esc(c.grade) +
          "</td><td>" + CORE.gradePoints(scale, c.grade).toFixed(2) + "</td><td>" + c.credits + "</td></tr>");
      });
    });
    win.document.write("<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Scholarics GPA Report</title>" +
      ...
      "<p class='sub'>Generated by Scholarics \u00B7 scholarics.com \u00B7 " + scale.label + " \u00B7 " +
      new Date().toLocaleDateString() + "</p>" +
      "<div class='hero'>" +
      "<div><div class='n'>" + fmtGpa(cg.gpa) + "</div><div class='l'>Cumulative GPA / " + scale.max.toFixed(1) + "</div></div>" +
