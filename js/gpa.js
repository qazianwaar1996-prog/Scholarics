(function () {
  ...
  function gradeCell(r) {
    ...
    var step = scale === "points" ? "0.1" : "1";
    /* Use text type to allow decimal input on all devices, validate manually */
    return `<input class="input tnum c-grade" data-f="grade" type="number" min="0" max="${max}" step="${step}" value="${r.grade}" placeholder="Grade (0–${max})">`;
  }
  function render() {
    var container = $("#rows");
    if (!container) return;
  ...
  function compute() {
    ...
    rows.forEach(function(r) {
      ...
      if (c < 0) c = 0;
      if (c > 50) c = 50; /* max 50 credits per course is a sane upper bound */
      if (c > 0) {
        cr += c;
        qp += (resolve(r) * c);
      }
